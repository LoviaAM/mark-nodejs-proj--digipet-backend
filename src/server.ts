...
import { hatchDigipet, walkDigipet, trainDigipet,ignoreDigipet, feedDigipet } from "./digipet/controller";
...
app.get("/digipet/feed", (req, res) => {
  ...
  if (getDigipet()) {
    feedDigipet();
    res.json({
      message: "You have fed your digipet",
      digipet: getDigipet(),
    });
  } else {
    res.json({
